...
const ACTIVITY_ELEMENTS = new Set<HudElement>(['tools', 'agents', 'todos', 'motif']);
...
function renderElementLine(ctx: RenderContext, element: HudElement): string | null {
  ...
  switch (element) {
    ...
    case 'version':
      return display?.showClaudeCodeVersion ? renderVersionElement(ctx) : null;
    case 'speed':
      return display?.showSpeed ? renderSpeedElement(ctx) : null;
    case 'duration':
      return display?.showDuration === false ? null : renderDurationElement(ctx);
    case 'customLabel':
      return renderCustomLabelElement(ctx);
    case 'context':
    ...
    case 'motif':
      return renderMotifLine(ctx);
    case 'diff':
      return renderDiffElement(ctx);
  }
}

/**
 * Render an element for the `lines` config path.
 * Presence in `lines` = intent to show, so skip display.show* gates.
 */
function renderElementForLines(ctx: RenderContext, element: HudElement): string | null {
  const withDisplay = (overrides: Record<string, unknown>) => ({
    ...ctx,
    config: {
      ...ctx.config!,
      display: { ...ctx.config!.display, ...overrides },
    },
  });
  ...
  switch (element) {
    case 'project':
      return renderProjectLine(ctx);
    case 'model':
      return renderModelElement(withDisplay({ showModel: true }));
    case 'path':
      return renderPathElement(withDisplay({ showProject: true }));
    case 'git': {
      const gitCtx = {
        ...ctx,
        config: {
          ...ctx.config!,
          gitStatus: { ...ctx.config!.gitStatus, enabled: true },
        },
      };
    ...
    case 'version':
      return renderVersionElement(withDisplay({ showClaudeCodeVersion: true }));
    case 'speed':
      return renderSpeedElement(withDisplay({ showSpeed: true }));
    case 'duration':
      return renderDurationElement(withDisplay({ showDuration: true }));
    case 'customLabel':
      return renderCustomLabelElement(ctx);
    case 'context':
      return renderIdentityLine(ctx);
    ...
    case 'memory': {
      ...
      const memCtx = {
        ...
        config: {
          ...ctx.config!,
          lineLayout: 'expanded' as const,
          display: { ...ctx.config!.display, showMemoryUsage: true },
        },
      };
      return renderMemoryLine(memCtx);
    }
    case 'environment':
      return renderEnvironmentLine(withDisplay({ showConfigCounts: true }));
    case 'tools':
...
function renderFromLines(ctx: RenderContext): Array<{ line: string; isActivity: boolean }> {
  const linesDef = ctx.config!.lines!;
  const result: Array<{ line: string; isActivity: boolean }> = [];

  for (const row of linesDef) {
    const rendered: string[] = [];
    let rowIsActivity = false;
    const overflowLines: string[] = [];

    for (const element of row) {
      const output = renderElementForLines(ctx, element);
      if (!output) continue;

      if (ACTIVITY_ELEMENTS.has(element)) {
        rowIsActivity = true;
      }

      // Handle multi-line elements (e.g., agents returns \n-joined lines)
      const parts = output.split('\n');
      rendered.push(parts[0]);
      if (parts.length > 1) {
        overflowLines.push(...parts.slice(1));
      }
    }
    ...
    if (rendered.length > 0) {
      result.push({
        line: rendered.join(' │ '),
        isActivity: rowIsActivity,
      });
    }

    for (const overflow of overflowLines) {
      result.push({ line: overflow, isActivity: rowIsActivity });
    }
  }
