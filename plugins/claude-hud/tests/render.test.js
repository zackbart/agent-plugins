...
// --- lines config rendering tests ---

test('lines config renders elements in specified order', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.config.lines = [['context'], ['project']];
  const lines = captureRenderLines(ctx);
  const contextIdx = lines.findIndex(l => l.includes('Context'));
  const projectIdx = lines.findIndex(l => l.includes('my-project'));
  assert.ok(contextIdx >= 0 && projectIdx >= 0);
  assert.ok(contextIdx < projectIdx, 'context should appear before project');
});

test('lines config joins multiple elements on one line', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.config.lines = [['project', 'context']];
  const lines = captureRenderLines(ctx);
  assert.ok(lines.find(l => l.includes('my-project') && l.includes('Context')));
});

test('lines config skips null elements gracefully', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.config.lines = [['project', 'tools']];
  const lines = captureRenderLines(ctx);
  assert.ok(lines.find(l => l.includes('my-project')));
});

test('lines config omits empty lines', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.config.lines = [['tools'], ['project']];
  const lines = captureRenderLines(ctx);
  assert.ok(lines[0].includes('my-project'));
});

test('lines config enables activity elements without show flags', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.config.display.showTools = false;
  ctx.config.display.showTodos = false;
  ctx.transcript.tools = [{ tool_use_id: 't1', name: 'Read', startTime: Date.now() - 2000, status: 'completed', duration: '1.0s' }];
  ctx.transcript.todos = [{ content: 'Test todo', status: 'in_progress' }];
  ctx.config.lines = [['tools'], ['todos']];
  const output = captureRenderLines(ctx).join('\n');
  assert.ok(output.includes('Read'), 'tools should render');
  assert.ok(output.includes('Test todo'), 'todos should render');
});

test('lines config enables memory without showMemoryUsage', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.config.display.showMemoryUsage = false;
  ctx.memoryUsage = { usedBytes: 500 * 1024 * 1024, totalBytes: 1024 * 1024 * 1024, usedPercent: 49 };
  ctx.config.lines = [['memory']];
  const output = captureRenderLines(ctx).join('\n');
  assert.ok(output.includes('RAM') || output.includes('49%'));
});

test('lines config enables environment without showConfigCounts', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.config.display.showConfigCounts = false;
  ctx.claudeMdCount = 2;
  ctx.mcpCount = 3;
  ctx.config.lines = [['environment']];
  const output = captureRenderLines(ctx).join('\n');
  assert.ok(output.includes('CLAUDE.md') || output.includes('MCP'));
});

test('elementOrder still works when lines absent', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.config.elementOrder = ['context', 'usage', 'project'];
  const lines = captureRenderLines(ctx);
  const contextLine = lines.findIndex(l => l.includes('Context'));
  const projectLine = lines.findIndex(l => l.includes('my-project'));
  assert.ok(contextLine < projectLine);
});

// --- sub-element rendering tests ---

test('renderModelElement returns model badge', () => {
  const ctx = baseContext();
  const result = renderModelElement(ctx);
  assert.ok(result && stripAnsi(result).includes('[Opus'));
});

test('renderPathElement returns project path', () => {
  const ctx = baseContext();
  ctx.stdin.cwd = '/home/user/my-project';
  const result = renderPathElement(ctx);
  assert.ok(result && stripAnsi(result).includes('my-project'));
});

test('renderGitElement returns git status', () => {
  const ctx = baseContext();
  ctx.gitStatus = { branch: 'main', isDirty: true, ahead: 0, behind: 0, fileStats: null };
  const result = renderGitElement(ctx);
  assert.ok(result && stripAnsi(result).includes('git:(main'));
});

test('renderSessionElement returns session name', () => {
  const ctx = baseContext();
  ctx.config.display.showSessionName = true;
  ctx.transcript.sessionName = 'my-session';
  assert.ok(stripAnsi(renderSessionElement(ctx)).includes('my-session'));
});

test('renderVersionElement returns CC version', () => {
  const ctx = baseContext();
  ctx.config.display.showClaudeCodeVersion = true;
  ctx.claudeCodeVersion = '2.1.81';
  assert.ok(stripAnsi(renderVersionElement(ctx)).includes('CC v2.1.81'));
});

test('renderDurationElement returns duration', () => {
  const ctx = baseContext();
  ctx.sessionDuration = '12m';
  assert.ok(stripAnsi(renderDurationElement(ctx)).includes('12m'));
});

test('renderCustomLabelElement returns custom text', () => {
  const ctx = baseContext();
  ctx.config.display.customLine = 'hello';
  assert.ok(stripAnsi(renderCustomLabelElement(ctx)).includes('hello'));
});

test('sub-elements render independently in lines config', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.gitStatus = { branch: 'main', isDirty: false, ahead: 0, behind: 0, fileStats: null };
  ctx.config.lines = [['model'], ['path', 'git'], ['context']];
  const lines = captureRenderLines(ctx);
  assert.ok(lines.length >= 3);
  assert.ok(lines[0].includes('[Opus'));
  assert.ok(lines[1].includes('my-project') && lines[1].includes('git:(main'));
  assert.ok(lines[2].includes('Context'));
});

test('sub-elements bypass display.show* gates in lines', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.config.display.showModel = false;
  ctx.config.display.showProject = false;
  ctx.config.display.showSessionName = false;
  ctx.config.display.showClaudeCodeVersion = false;
  ctx.config.display.showDuration = false;
  ctx.config.gitStatus.enabled = false;
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.gitStatus = { branch: 'feat', isDirty: false, ahead: 0, behind: 0, fileStats: null };
  ctx.claudeCodeVersion = '2.0.0';
  ctx.sessionDuration = '5m';
  ctx.transcript.sessionName = 'test-session';
  ctx.config.lines = [['model'], ['path'], ['git'], ['session'], ['version'], ['duration']];
  const output = captureRenderLines(ctx).join('\n');
  assert.ok(output.includes('[Opus'), 'model bypasses showModel');
  assert.ok(output.includes('my-project'), 'path bypasses showProject');
  assert.ok(output.includes('git:(feat'), 'git bypasses gitStatus.enabled');
  assert.ok(output.includes('test-session'), 'session bypasses showSessionName');
  assert.ok(output.includes('CC v2.0.0'), 'version bypasses showClaudeCodeVersion');
  assert.ok(output.includes('5m'), 'duration bypasses showDuration');
});

test('project bundle still works with sub-elements available', () => {
  const ctx = baseContext();
  ctx.config.lineLayout = 'expanded';
  ctx.stdin.cwd = '/home/user/my-project';
  ctx.gitStatus = { branch: 'main', isDirty: true, ahead: 0, behind: 0, fileStats: null };
