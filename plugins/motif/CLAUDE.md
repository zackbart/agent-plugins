...
- Stage 4 (Validate) runs **two validators in parallel, every time**: the Claude `validator` subagent and a Codex second-opinion pass over the same change set (`git diff --name-only <baseCommit>`). Unlike the Stage 2 codex critic, the validate pass is **ungated** — no flag, runs whenever the `codex` binary is present. Same invocation (`codex exec -s read-only ... 2>/dev/null`, ~5 min bound, honors `~/.codex/config.toml`, no `-m`). Codex's briefing instructs it to follow `agents/validator.md` exactly (the single source of truth for validator contract: verdict, `[ISSUE]/[WARNING]/[NOTE]` tags, `> Verdict:` summary line). Findings from both fold into one merged report, deduped with agreement marked (`[ISSUE] (Claude + Codex)`). Non-gating: the user's fix/done choice is the only gate; any Codex failure (missing binary, non-zero exit, timeout, truncated) silently degrades to Claude-only validation.
