...
CODEX_OUTPUT=$(codex exec \
  ...
  "$(cat /tmp/motif-critic-briefing.txt)" 2>&1)
CODEX_EXIT=$?
rm -f /tmp/motif-critic-briefing.txt
printf '%s\n' "# Critic Review (Codex / gpt-5.4)" "" "$CODEX_OUTPUT" > .motif/critic-output.md
[ -f .motif/critic-output.md ] && echo "OK: $(wc -l < .motif/critic-output.md) lines written" || echo "WRITE FAILED"
echo "--- Exit code: $CODEX_EXIT ---"
echo "$CODEX_OUTPUT"
```

**IMPORTANT: Do NOT split this into separate Bash calls.** The variable `$CODEX_OUTPUT` only exists within the single call. If you run the CLI and write in separate calls, you must re-embed the content as a string literal, which breaks on special characters.
